// JavaScript script to update ALL HII stations with Amphure and Province information
...
async function updateHIILocations() {
  ...
  logger.info('Starting update of ALL HII station locations with new API key...');
  ...
  try {
    ...
    logger.info(`Found ${result.rows.length} HII stations ${options.forceUpdate ? '' : 'without location data'} in database`);
    ...
    if (options.saveToFile) {
      const resultsFile = 'all_hii_location_update_results.json';
