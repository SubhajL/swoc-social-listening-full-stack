...
async function reverseGeocode(lat, lng, options, logger) {
  ...
  try {
    ...
    // Try both the environment variable and the hardcoded key that worked for TMD
    const tmdKey = "AIzaSyDQG_0Uf-Hn-aCv9UKL-XyxbvVDOLj5OJk"; // From .env file
    logger.debug(`TMD Key from .env: ${tmdKey.substring(0, 10) + '...'}`);
    
    // First try with the environment variable
    logger.debug(`Trying with environment API key...`);
    ...
    if (response.data.status !== 'OK') {
      ...
      // If the first attempt failed, try with the TMD key
      if (response.data.status === 'REQUEST_DENIED' && tmdKey !== apiKey) {
        logger.debug(`First attempt failed. Trying with TMD key...`);
        const tmdUrl = `https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key=${tmdKey}&language=th&result_type=administrative_area_level_1|administrative_area_level_2|administrative_area_level_3`;
        logger.debug(`TMD Request URL: ${tmdUrl}`);
        
        try {
          const tmdResponse = await axios.get(tmdUrl);
          logger.debug(`TMD API Response status: ${tmdResponse.data.status}`);
          
          if (tmdResponse.data.status === 'OK') {
            logger.info(`Successfully geocoded with TMD key`);
            return processGeocodingResponse(tmdResponse.data, logger);
          } else {
            logger.warn(`TMD key also failed with status: ${tmdResponse.data.status}`);
            if (tmdResponse.data.error_message) {
              logger.debug(`TMD API Error message: ${tmdResponse.data.error_message}`);
            }
          }
        } catch (tmdError) {
          logger.error(`Error with TMD key request: ${tmdError.message}`);
        }
      }
...
async function updateHIILocations() {
  ...
  logger.info('Starting update of HII station locations with enhanced debugging...');
  ...
  try {
    ...
    if (options.saveToFile) {
      const resultsFile = 'hii_location_update_debug3_results.json';
