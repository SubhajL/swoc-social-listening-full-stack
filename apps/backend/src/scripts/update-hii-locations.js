...
class Options {
  constructor() {
    this.batchSize = 50; // Process stations in batches to avoid overwhelming the API
...
// This is a simplified approach - for a more accurate check, we would need a detailed coastline dataset
function isOnLand(lat, long) {
  // This is a very simplified check - in reality, we would need a more sophisticated approach
  // For now, we'll assume coordinates are in water if they're in certain regions of the Gulf
...
async function updateHIILocations() {
  ...
  logger.info('Starting update of HII station locations...');
