...
async function exportWithPrecision() {
  ...
  try {
    ...
    reservoirs.slice(0, 3).forEach(r => {
      console.log(`${r.reservoir_id}: lat=${r.reservoir_lat} (length: ${r.reservoir_lat.length}), long=${r.reservoir_long} (length: ${r.reservoir_long.length})`);
    });
