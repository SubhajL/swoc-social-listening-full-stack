// Script to update tambon, amphure, and province for all stations in thaiwater_tele_stations
import pg from 'pg';
...
class Options {
  constructor() {
    ...
    this.resultsFile = 'station_location_update_results.json'; // File to save results to
 ...
 */
async function updateStationLocations() {
  logger.info('Starting station location update process...');
  ...
  try {
    ...
    // Query stations without location data
    const stationsWithoutLocation = await queryStationsWithoutLocation(pool);
    logger.info(`Found ${stationsWithoutLocation.length} stations without complete location data`);
    
    // Query stations with incorrect location data
    const stationsWithIncorrectLocation = await queryStationsWithIncorrectLocation(pool);
    logger.info(`Found ${stationsWithIncorrectLocation.length} stations with potentially incorrect location data`);
    
    // Combine stations to update
    const stationsToUpdate = [...stationsWithoutLocation, ...stationsWithIncorrectLocation];
    logger.info(`Total stations to update: ${stationsToUpdate.length}`);
    
    // Process stations in batches
    const results = await processStationsInBatches(stationsToUpdate, apiKey, pool, options);
    ...
    await pool.end();
    logger.info('Station location update process completed');
  }
}

/**
 * Query stations without complete location data
 * 
 * @param {pg.Pool} pool - Database connection pool
 * @returns {Promise<Array>} - Array of stations without location data
 */
async function queryStationsWithoutLocation(pool) {
  logger.info('Querying stations without complete location data...');
  ...
  const query = `
    ...
    WHERE 
      (province IS NULL OR amphure IS NULL OR tambon IS NULL)
  ...
  return result.rows.map(station => ({
    ...
    update_reason: 'missing_location'
  }));
}

/**
 * Query stations with potentially incorrect location data
 * 
 * @param {pg.Pool} pool - Database connection pool
 * @returns {Promise<Array>} - Array of stations with potentially incorrect location data
 */
async function queryStationsWithIncorrectLocation(pool) {
  logger.info('Querying stations with potentially incorrect location data...');
  
  // List of known incorrect mappings
  const incorrectMappings = [
    { id: 2236, name: 'เกาะสมุย', expected_province: 'สุราษฎร์ธานี', current_province: 'นครศรีธรรมราช' },
    { id: 2233, name: 'นราธิวาส', expected_province: 'นราธิวาส', current_province: 'สงขลา' }
  ];
  
  const stationIds = incorrectMappings.map(mapping => mapping.id);
  ...
  return result.rows.map(station => {
    const mapping = incorrectMappings.find(m => m.id === station.tele_station_id);
    return {
      id: station.tele_station_id,
      name: station.tele_station_name,
      latitude: parseFloat(station.tele_station_lat),
      ...
      tambon: station.tambon,
      data_source: station.data_source,
      update_reason: 'incorrect_location',
      expected_province: mapping ? mapping.expected_province : null
    };
  });
}
  ...
  for (let i = 0; i < stations.length; i += options.batchSize) {
    ...
    for (const station of batch) {
      try {
        ...
        // Update the station in the database if enabled
        ...
        results.stations.push({
          ...station,
          status: locationInfo.administrative_areas.province ? 'updated' : 'skipped',
          reason: locationInfo.administrative_areas.province ? 'success' : 'geocoding_failed',
...
// Run the function
updateStationLocations().catch(error => {
