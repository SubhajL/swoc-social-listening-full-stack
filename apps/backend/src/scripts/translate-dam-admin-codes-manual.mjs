...
async function translateDamAdminCodes() {
  try {
    ...
    console.log('\nProvince mapping results:');
    for (const dam of processedRecords.slice(0, 5)) {
      console.log(`Dam: ${dam.DAM_Name}, Province code: ${dam.provincecode}, Province name: ${dam.provinceName}`);
    }
    ...
    // Debug output for basin mapping
    console.log('\nBasin mapping results:');
    for (const dam of processedRecords.slice(0, 5)) {
      console.log(`Dam: ${dam.DAM_Name}, Basin ID: ${dam.basinid}, Basin name: ${dam.basinName}`);
    }
