...
async function updateAndExportReservoirLocations() {
  ...
  try {
    ...
    try {
      ...
      // 1. First, get all reservoir records
      ...
      for (const record of getResult.rows) {
        try {
          ...
          // Determine data_source based on reservoir_name
          const dataSource = record.reservoir_name && record.reservoir_name.startsWith('อ่างเก็บน้ำ') 
            ? 'reservoir' 
            : (record.data_source || 'excel');
          ...
          `, [formattedId, dataSource, numericId]);
      ...
      const exportResult = await client.query('SELECT * FROM reservoir_locations ORDER BY reservoir_id');
