// Script to clean up amphure names by removing the "อำเภอ" prefix
import pg from 'pg';
 ...
 */
async function cleanAmphureNames() {
  console.log('Starting amphure name cleanup process...');
  ...
  try {
    // Query all stations with amphure names that start with "อำเภอ"
    const stationsToClean = await queryStationsWithPrefix(pool);
    ...
    fs.writeFileSync('amphure_cleanup_results.json', JSON.stringify(results, null, 2));
    console.log(`Results saved to amphure_cleanup_results.json`);
...
/**
 * Query stations with amphure names that start with "อำเภอ"
 * 
 * @param {pg.Pool} pool - Database connection pool
 * @returns {Promise<Array>} - Array of stations
 */
async function queryStationsWithPrefix(pool) {
  console.log('Querying stations with amphure names that start with "อำเภอ"...');
...
async function cleanAndUpdateAmphureNames(pool, stations) {
  ...
  for (const station of stations) {
    try {
      ...
      // Remove "อำเภอ" prefix with or without space
