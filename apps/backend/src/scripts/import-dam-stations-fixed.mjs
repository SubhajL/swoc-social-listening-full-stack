    ...
    try {
      ...
      for (const row of data) {
        try {
          ...
          const amphure = row['อำเภอ']; // Amphure
          ...
          if (!reservoirName || !province || !amphure) {
          ...
          if (checkResult.rows.length > 0) {
            ...
            const updateResult = await client.query(updateQuery, [
              province, amphure, 'dam_excel_v2', reservoirName
            ]);
            ...
            const insertResult = await client.query(insertQuery, [
              nextId, reservoirName, province, amphure, 'dam_excel_v2', reservoirId, JSON.stringify(metadata)
            ]);
