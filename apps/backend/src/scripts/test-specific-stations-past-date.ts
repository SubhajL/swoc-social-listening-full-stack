...
async function testSpecificStationsWithPastDate() {
  try {
    console.log('Starting RID API test for specific stations with past dates...');
    ...
    // Try different dates (past dates in Thai Buddhist calendar format)
    const pastDates = [
      { date: '14/03/2568', description: 'Yesterday' },
      { date: '10/03/2568', description: '5 days ago' },
      { date: '01/03/2568', description: 'Beginning of month' },
      { date: '15/02/2568', description: 'Last month' },
      { date: '15/01/2568', description: '2 months ago' }
    ];
    ...
    for (const station of maeTaengStations) {
      ...
      for (const pastDate of pastDates) {
        console.log(`\n--- Testing with date: ${pastDate.date} (${pastDate.description}) ---`);
        
        // Prepare request body
        const requestBody = {
          hydro: {
            StationID: station.id,
            TimeStart: pastDate.date
          }
        };
        ...
        try {
          ...
          if (response.status === 200) {
            ...
            if (Array.isArray(data)) {
              console.log(`Found ${data.length} telemetry readings for station ${station.id} on ${pastDate.date}`);
              ...
              if (data.length > 0) {
                ...
                // If we found data, break out of the date loop for this station
                break;
              }
            } else {
...
// Run the test
testSpecificStationsWithPastDate().catch(console.error); 