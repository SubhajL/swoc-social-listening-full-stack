...
// Function to update coordinates in database
async function updateCoordinatesInDatabase(excelData) {
  ...
  try {
    ...
    for (const row of excelData) {
      ...
      try {
        const result = await client.query(updateQuery, [latitude, longitude, reservoirId]);
        
        if (result.rowCount > 0) {
          console.log(`[CoordinateUpdate] Updated coordinates for ${reservoirId}: (${latitude}, ${longitude})`);
          updatedCount++;
        } else {
  ...
  try {
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      for (const reservoir of batch) {
        // Use the correct column names: reservoir_lat and reservoir_long
    ...
    const outputPath = path.resolve(__dirname, '../../../../reservoir_locations_precise.json');
...
async function main() {
  try {
    ...
    // Step 2: Update coordinates in database
