...
async function checkStation420() {
  ...
  try {
    ...
    if (stationResult.rows.length === 0) {
      console.log('Station 420 not found in the database.');
      return;
    }
    ...
    // Get all rainfall data for station 420
    const rainfallQuery = `
      SELECT 
        rainfall_datetime, 
        rainfall10m, 
    ...
    console.log(`\nAll rainfall records for station 420 (${rainfallResult.rows.length} records):`);
    ...
    if (rainfallResult.rows.length > 0) {
      ...
      if (nineAMRecords.length > 0) {
        ...
        console.log("No 9:00 AM records found for station 420.");
      }
    } else {
      console.log('No rainfall data available for station 420.');
    }
