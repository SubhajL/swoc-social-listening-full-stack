...
export async function fetchCoordinatesWithRetry(searchQueries: string[]): Promise<{ lat: number; lng: number } | null> {
  const maxRetries = 5;
...
export async function updateProvinceCoordinates(client: pg.PoolClient): Promise<void> {
  try {
...
export async function updateAmphureCoordinates(client: pg.PoolClient): Promise<void> {
  try {
