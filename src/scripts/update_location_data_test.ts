...
async function updateProvinceCoordinates(client: pg.PoolClient): Promise<void> {
  try {
    ...
    for (let i = 0; i < provinces.rows.length; i++) {
      ...
      try {
        ...
        if (coordinates) {
          ...
          logProgress({
            message: `Updated coordinates for ${province.name_en}`,
            data: {
        ...
        logProgress({
          message: `Error processing ${province.name_en}`,
          data: {
            error: error instanceof Error ? error.message : String(error)
          }
        }, 'error');
    ...
    logProgress({
      message: 'Error in updateProvinceCoordinates',
  ...
  try {
    ...
    for (let i = 0; i < amphures.rows.length; i++) {
      ...
      try {
        ...
        logProgress({
          message: 'Error processing amphure',
          data: {
            error: error instanceof Error ? error.message : String(error)
          }
        }, 'error');
    ...
    logProgress({
      message: 'Error in updateAmphureCoordinates',
      data: { error: String(error) }
    }, 'error');
    throw error;
  }
